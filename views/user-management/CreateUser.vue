...
import { required, email ,alpha,numeric,helpers} from "vuelidate/lib/validators";
...
export default {
  ...
  methods: {
    ...
    createUser() {
      ...
      if (this.$v.$invalid) {
        this.showToast("Please fill all required fields without Spaces.!", "warning");
        return;
      }
      if (this.userId != null) {
        ...
        AuthService.updateUser(this.personalInfo)
          .then((res) => {
            if (res.data.responseIdentifier == "Success") {
              this.$emit("refreshSDT", "User Updated successfully.");
