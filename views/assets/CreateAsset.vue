<template>
  <div class="pt-32 pb-80">
    ...
    <div class="main-page pt-32">
            ...
            >
              ...
              >
                <SInput
                  ...
                  "
                  v-model="input.value"
                  :error="getFormErrorMessage(input.validation?$v.customCategoryFields.$each[k].value:'')"
                />
              </div>
              <div v-else-if="input.elementConfig.label !== '# of Washes'">
                <SSelect
                  :label="
                    input.validation
                      ? input.elementConfig.label + '*'
      ...
      <div></div>
      <div class="right-column">
        <Sidebar :assetObj="assetObj" :currency="currency" />
        <SButton
          :dropdown="false"
          rounded
          v-if="assetID == undefined"
  ...
  methods: {
    ...
    updateAsset() {
      ...
      if (this.customCategoryFields.length !== 0) {
        for (let index = 0; index < this.customCategoryFields.length; index++) {
          let assetField = {};
          assetField.id = this.assetObj.assetFields[
            this.customCategoryFields[index].id
          ].id;
          assetField.uuid = this.assetObj.assetFields[
            this.customCategoryFields[index].id
          ].uuid;
          assetField.assetUUID = this.assetObj.assetFields[
            this.customCategoryFields[index].id
          ].assetUUID;
          assetField.fieldTemplateId = this.selectAssetCategory.fieldTemplate.uuid;
          assetField.fieldId = this.customCategoryFields[index].id;
  ...
  validations: function() {
    return {
      ...
      assetObj: {
        ...
        warranty: { required, maxLength: maxLength(190) },
